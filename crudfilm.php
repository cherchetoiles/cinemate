...
<!-- /TAILWIND CONFIG -->
    ...
    <title>AllosimplonAdminFilm</title>

</head>
<body class="w-full h-autp flex flex-col bg-violet pb-16">
...
<div class="w-full h-full flex flex-col items-center">
    ...
    <div class="flex-col  items-center text-center font-bold font-sans text-white text-4xl">
        <h1 class="mt-4 capitalize">Films</h1>
    ...
    <div class="flex mb-2 justify-end justify-items-end text-end mr-14 text-white font-sans text-xl font-bold w-full h-auto">
        <label for="my-modal-52" class="cursor-pointer">
   ...
   <?php
    ...
    $sql = "SELECT *,GROUP_CONCAT(id_artiste) FROM `film` f JOIN `jouer` j ON f.id_film = j.id_film GROUP BY(f.id_film)";  
    ...
    $sql = "SELECT id_artiste, nom, prenom FROM artiste";
    $artiste = $db->prepare($sql);
    $artiste->execute();
    ?>
    
   
    <div class="grid grid-cols-3 grid-rows-3 h-auto bg-violet ">

    <?php
    foreach($result->fetchAll() as $film){
        ?>

     <!--USER CARD-->
    
    <div class="m-4 w-auto h-auto py-4 text-white flex bg-violettrans/50 rounded-big  items-center px-3 justify-center align-middle" >
        <!-- PHOTO AND ROLE -->
        <div class="flex flex-col justify-center items-center  text-center mx-1 mt-2">
            <div alt="image d'avatar"  class="w-40 h-40 bg-auto rounded-big bg-[url('<?= $film ['img_film']?>')] bg-cover"></div>
            <h6 class="text-2xl font-bold font-sans text-white mt-1"><?= $film ['title']?></h6>
            <p class="text-base font-regulard font-body mt-[-10px]"><?= 'category'?></p>
        ...
        <!-- PHOTO AND ROLE -->
            ...
            <div class="flex flew-row mr-2 mb-6 justify-end">
                ...
                <!--/MODIFIE -->
                <!-- DELETE -->
                <label for="my-modal-12" class="cursor-pointer">
...
<?php include('form/deletefilm.php')?>
<?php include('form/createfilm.php')?>
